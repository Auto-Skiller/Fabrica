...
export const workspaceApi = {
  ...
  writeWorkspaceFile: (filePath: string, content: string, options?: { isImport?: boolean; type?: string; level?: any; description?: string; when_to_use?: string; triggers?: string[]; flagged_as_action?: boolean }) => request<{ ok: boolean; path: string; size: number }>('/api/workspace/file/write', { method: 'POST', body: JSON.stringify({ path: filePath, content, ...options }) }),
  flagActionItem: (itemPathOrItem: string | any) => request<{ ok: boolean }>('/api/workspace/flag-action', { method: 'POST', body: JSON.stringify(typeof itemPathOrItem === 'string' ? { path: itemPathOrItem } : { item: itemPathOrItem }) }),
